...
If multiple branches write to the same state key, you need to decide how those values should be combined. Without a reducer, the default behavior is last-write-wins — acceptable for some cases, but not for result aggregation.
